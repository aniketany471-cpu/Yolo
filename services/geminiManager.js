// @google/genai lazy-loaded to defer ~40MB until first Gemini request
...
export async function requestGemini({ source = "unknown", requestId = "global", apiKey, model, contents, config = {}, tools, attemptType = "primary" }) {
  ...
  try {
    ...
    // Clean up per-request Maps so they don't accumulate across thousands of messages
    if (requestId !== "global") {
      perMessage.delete(requestId);
      REQUEST_KEY_LOCK.delete(requestId);
    }
  }
}
