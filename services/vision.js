...
export async function analyzeTelegramImageWithGemini(client, message, geminiApiKey) {
  ...
  for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    ...
    if (modelIndex > 0) {
      console.log("[vision] switching to fallback model gemini-1.5-flash");
    }

    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        console.log(`[vision] request started model=${model} attempt=${attempt}/3`);
        ...
        if (attempt < 3) {
          ...
          console.log(`[vision] retry ${attempt}/3`);
          await sleep(waitMs);
        }
      }
    }
