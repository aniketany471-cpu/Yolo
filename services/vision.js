...
export async function analyzeTelegramImageWithGemini(client, message, geminiApiKey) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    for (const model of models) {
      try {
        console.log(`[vision] request started attempt=${attempt} model=${model}`);
        ...
        if (!parsed) {
          console.warn("[vision] parse failure");
        ...
        if (!valid) {
          console.warn("[vision] validation failure");
          lastReason = "validation failure";
          continue;
        }
        ...
        const retriable = msg.includes("503") || msg.includes("429") || msg.includes("overload") || msg.includes("unavailable") || msg.includes("timeout") || msg.includes("empty response");
        lastReason = msg.slice(0, 120);
        console.warn(`[vision] failure attempt=${attempt} model=${model} retriable=${retriable}`);
        if (!retriable && model === "gemini-1.5-flash") break;
        if (!retriable) continue;
      }
    }
    if (attempt < 3) {
      const waitMs = RETRY_DELAYS_MS[attempt - 1] || 1500;
      console.log(`[vision] retry count=${attempt} waitMs=${waitMs}`);
      await sleep(waitMs);
    }
  }

  console.warn(`[vision] final failure reason=${lastReason}`);
