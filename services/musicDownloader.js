...
function buildArgs(url, outputPath, opts = {}) {
  const { clientArgs, userAgent, format, cookies, ffmpegPath, noCookies } = opts;
  ...
  // Only pass cookies for clients that support them (noCookies=true for android/ios)
  if (!noCookies && cookies && fs.existsSync(cookies)) args.push("--cookies", cookies);
...
async function strategyYtdlpBinary(url, outputPath, options, onLog) {
  ...
  // Strategy order:
  //   1. ios — Apple API, no PO token, no nsig challenge; most reliable on Railway
  //   2. web_creator+cookies — bgutil plugin (installed via nixpacks) handles PO token
  //   3. mweb+cookies — secondary web fallback
  const hasCookies = !!(cookies && fs.existsSync(cookies));
  const clients = [
    // Primary: ios — Apple API, no PO token, no signature challenge
    {
      name: "ios",
    ...
    // web_creator and mweb only attempted when user has pasted cookies
    ...(hasCookies ? [{
      name: "web_creator",
