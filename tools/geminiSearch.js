...
function parseGroundedFact(text, type) {
  ...
  if (type === 'sports') {
    ...
    if (status === 'No match' || (!teams || !teams.length)) {
      // No match found — return null so the caller can decide how to respond
      console.warn('[gemini-search] Sports: Gemini returned No match');
      return null;
    }
...
export async function geminiGroundedSearch(query, apiKey, type = 'general', opts = {}) {
  ...
  try {
    ...
    if (type === 'sports' || type === 'weather') {
      ...
      // JSON extraction failed — return raw text (better than nothing)
