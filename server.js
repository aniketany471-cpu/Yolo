...
// FIX-3: Zero-key fallback scraper (Google HTML + RSS, no API key required).
// Used as step 3 in performWebSearch when Serper and Tavily both fail/missing.
let scraperMod = null;
try {
  scraperMod = await import("./services/scraper.js");
  console.log("[scraper] Zero-key fallback scraper loaded OK");
} catch (_scraperErr) {
  console.warn("[scraper] Fallback scraper unavailable:", _scraperErr.message);
}
...
// FIX-5: Pass the existing db instance instead of a path so memoryStore shares
// the single SQLite connection and does not open a second handle to the same file.
...
async function performWebSearch(query, config, deep = false) {
  ...
  // FIX-3: Scraper — zero-key fallback (Google HTML + RSS, no API key required).
  // Activated when both Serper and Tavily are unavailable or return nothing.
  if (scraperMod) {
    try {
      const intentInfo = needsSearch ? needsSearch(query) : { intent: 'general' };
      const intent = intentInfo.intent || 'general';
      let scraped = null;

      if (intent === 'weather') {
        // Strip query words to extract raw location for the weather API.
        const loc = query
          .replace(/\b(weather|temp(?:erature)?|forecast|climate)\s*(in|for|at|of)?\s*/gi, '')
          .replace(/\b(today|now|current|right now|rn|live)\b/gi, '')
          .trim() || query;
        const w = await scraperMod.getWeatherFree(loc);
        scraped = w?.message || null;
      } else if (intent === 'news') {
        const n = await scraperMod.searchNews(query);
        scraped = n?.formatted || null;
      } else {
        const w = await scraperMod.searchWeb(query, 4);
        scraped = w?.formatted || null;
      }

      if (scraped) {
        console.log(`[search] Scraper zero-key fallback OK — intent=${intent}`);
        return scraped;
      }
    } catch (e) {
      console.warn(`[search] Scraper fallback error: ${e.message}`);
    }
  }

  // FIX-4: All paths exhausted — log explicitly so this is never silent.
  console.warn(`[search] All search providers exhausted for: "${query.slice(0, 80)}"`);
  return "";
}
...
function formatAiMessage(text) {
  ...
  // FIX-7: Raised threshold from 100 → 250 words so normal conversational replies
  // are not wrapped in a collapsible blockquote that requires an extra tap to read.
  if (wordCount > 250) {
    ...
    if (p.key && p.key !== "undefined" && p.key !== "null" && p.key.length > 5) {
      try {
        const resRaw = await p.fn(
          ...
          p.key,
          context,
          `${timeContext} ${systemPrompt}${modelNudge ? " " + modelNudge : ""} ${searchContext ? "\n\n" + searchContext : ""}`
        );
        if (resRaw) {
...
const HS = {
  ...
  search:       () => _pick(["Looking through sources...", "Trying to verify this...", "Lemme check...", "Wait, checking the latest info...", "Yeah one sec...", "Reading about it...", "Checking that...", "Got something, hang on...", "Looking into it...", "On it...", "Let me pull that up..."]),
  ...
  vision:       () => _pick(["Reading the image...", "Let me take a look at that...", "On it...", "Checking what's in there..."]),
};
// ─────────────────────────────────────────────────────────────────────────────
...
async function startServer() {
  ...
  app.get("/api/state", (req, res) => {
    try {
      ...
      // Mask sensitive fields before sending to the frontend
      const SENSITIVE_KEYS = [
        'geminiKey', 'groqKey', 'openRouterKey', 'bluesmindsApiKey', 'xaiKey',
        'searchApiKey', 'serperKey',
        'telegramApiHash', 'telegramStringSession', 'youtube_cookies',
      ];
      const safeConfig = config ? { ...config } : config;
      if (safeConfig) {
...
\u23F1 ${video.timestamp}`,
  ...
  async function maybeHandleAutoReply(client2, message, config, myId, myUsername) {
    ...
    const hasPhotoMedia = !!(message.media?.className === 'MessageMediaPhoto' || message.media?.photo);
    if (!text && !hasPhotoMedia) return;
    if (text.startsWith("/") || text.startsWith(".")) return;

    // ── Vision: analyse incoming photo before AI dispatch ───────────────────
    let visionContext = "";
    if (hasPhotoMedia) {
      try {
        const photoBuffer = await client2.downloadMedia(message.media, {});
        if (photoBuffer && Buffer.isBuffer(photoBuffer)) {
          ...
          if (vRes?.ok && vRes?.data) {
            visionContext = `[IMAGE SENT BY USER]\n${vRes.data}\n[/IMAGE SENT BY USER]`;
            console.log(`[vision] Auto-reply photo analysed (${vRes.data.length} chars)`);
          } else {
            console.warn(`[vision] Photo analysis failed: ${vRes?.reason}`);
          }
        }
      } catch (vErr) {
        console.warn("[vision] Photo download/analysis error:", vErr?.message);
      }
    }
    // ────────────────────────────────────────────────────────────────────────
    // If a photo was sent but vision failed and there's no text, nothing to reply to
    if (!text && hasPhotoMedia && !visionContext) return;
    ...
    setTimeout(async () => {
      try {
        ...
        // Show smarter status: vision if photo, search if live data needed, else think
        const searchStatus = (config.searchEnabled === 1 && needsSearch)
          ? needsSearch(text).needs
          : false;
        await status.update(visionContext ? HS.vision() : searchStatus ? HS.search() : HS.think());
        // Build the final prompt — inject vision description when a photo was sent
        const aiPrompt = visionContext
          ? `${visionContext}\n\nUser message: ${text || "What do you see in this image?"}`
          : text;
    ...
    try {
      ...
      const messageHandler = async (event) => {
        try {
          ...
          const hasMediaPhoto = !!(message?.media?.className === 'MessageMediaPhoto' || message?.media?.photo);
          if (!message || (!message.message && !hasMediaPhoto)) return;
