...
try { db.exec("ALTER TABLE config ADD COLUMN sudoUsers TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN publicCommandsEnabled INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN blacklistedUsers TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN whitelistedUsers TEXT DEFAULT '';"); } catch(e) {}

// ... existing try-catch migrations ...
try { db.exec("ALTER TABLE config ADD COLUMN youtube_cookies TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN telegramApiId TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN telegramApiHash TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN telegramStringSession TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN globalCooldown INTEGER DEFAULT 3;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN perUserCooldown INTEGER DEFAULT 10;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN maxConcurrentTasks INTEGER DEFAULT 2;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN aiEnabled INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN aiProvider TEXT DEFAULT 'gemini';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN geminiKey TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN groqKey TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN openRouterKey TEXT;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoDeleteCommands INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoDeleteDelay INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoDeleteWhitelist TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyDM INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyMention INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN typingSimulation INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN conversationMemory INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyDelayMin INTEGER DEFAULT 3;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyDelayMax INTEGER DEFAULT 15;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyPersonality TEXT DEFAULT 'You are a modern Telegram AI assistant. Reply intelligently, naturally, and concisely. Avoid robotic greetings, filler text, and generic explanations. Format responses beautifully for Telegram. Use short readable paragraphs. Always prioritize useful and accurate answers.';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyWhitelist TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN autoReplyBlacklist TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN nsfwEnabled INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN nsfwPersonality TEXT DEFAULT 'You are a flirty, mature, and consenting adult friend.';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN searchEnabled INTEGER DEFAULT 0;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN searchProvider TEXT DEFAULT 'tavily';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN searchApiKey TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN aiMode TEXT DEFAULT 'intelligent';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN formattingEnabled INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN cleanupEnabled INTEGER DEFAULT 1;"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN bluesmindsApiKey TEXT DEFAULT '';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN activeModel TEXT DEFAULT 'gemini-1.5-flash';"); } catch(e) {}
try { db.exec("ALTER TABLE config ADD COLUMN deepThinking INTEGER DEFAULT 0;"); } catch(e) {}
...
const existingConfig = db.prepare("SELECT openRouterKey, aiProvider FROM config WHERE id = 1").get() as any;
if (!existingConfig?.openRouterKey || existingConfig.openRouterKey.length < 10) {
  db.prepare("UPDATE config SET openRouterKey = ?, aiProvider = 'openrouter' WHERE id = 1").run('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');
}

async function getGeminiResponse(prompt: string, apiKey: string, model: string = "gemini-1.5-flash", context: any[] = [], systemInstruction?: string) {
  try {
    ...
    const contents = context.length > 0 
    ...
    if (errMsg.includes("API key not valid") || errMsg.includes("invalid API key") || errMsg.includes("API_KEY_INVALID")) {
       console.warn("[Gemini] Auth Warning: The provided API key was rejected. Skipping to next provider.");
       return null;
    }
...
async function getGroqResponse(prompt: string, apiKey: string, model: string = "llama3-8b-8192", context: any[] = [], systemInstruction?: string) {
  try {
    const cleanKey = apiKey?.trim();
    ...
    const messages = context.length > 0
      ? [
          ...
          ...context.map(c => ({ role: c.role === 'model' ? 'assistant' : c.role, content: c.parts[0].text })), 
    ...
    if (!response.ok || !contentType?.includes("application/json")) {
      ...
      console.error(`[Groq] API Error (${response.status}, ${contentType}):`, err.substring(0, 500));
      return null;
    }
...
async function getOpenRouterResponse(prompt: string, apiKey: string, model: string = "google/gemini-2.0-flash-001", context: any[] = [], systemInstruction?: string) {
  try {
    const cleanKey = apiKey?.trim();
    if (!cleanKey || cleanKey === 'undefined' || cleanKey === 'null') return null;
    ...
    const response = await fetch("https://api.v1.openrouter.ai/chat/completions", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${cleanKey}`,
        "Content-Type": "application/json",
    ...
    if (!response.ok || !contentType?.includes("application/json")) {
      const err = await response.text();
      console.error(`[OpenRouter] API Error (${response.status}, ${contentType}):`, err.substring(0, 500));
      return null;
    }
...
async function getBluesMindsResponse(prompt: string, apiKey: string, model: string = "gemini-1.5-flash", context: any[] = [], systemInstruction?: string) {
  try {
    const cleanKey = apiKey?.trim();
    if (!cleanKey || cleanKey === 'undefined' || cleanKey === 'null') return null;
    ...
    if (!response.ok) {
      ...
      if (errText.includes("reached its end of life") || response.status === 410 || response.status === 503 || errText.includes("model_not_found") || errText.includes("no available channel")) {
        console.warn(`[AI] BluesMinds Warning (${response.status}): Model ${model} is discontinued or unavailable. Attempting fallback...`);
        
        const fallbackChain = ["gemini-1.5-pro", "gemini-2.0-flash-exp", "deepseek-chat", "gpt-4o-mini", "gemini-1.5-flash"];
...
async function performWebSearch(query: string, config: any, deep: boolean = false): Promise<string> {
  if (config.searchEnabled !== 1 || !config.searchApiKey) return "";
  
  try {
    const depth = deep ? "advanced" : "basic";
    const maxResults = deep ? 6 : 3;
    console.log(`[Search] Performing ${depth} web search for: "${query}" using Tavily...`);
...
function formatAiMessage(text: string): { text: string, parseMode: 'html' | 'markdown' } {
...
async function generateImage(prompt: string, apiKey: string, model: string = "flux") {
  try {
    const cleanKey = apiKey?.trim();
    if (!cleanKey) return null;

    const response = await fetch("https://api.bluesminds.com/v1/images/generations", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${cleanKey}`
...
async function getAIResponse(prompt: string, config: any, chatId?: string, userId?: string, isNSFWActive: boolean = false, forceDeep: boolean = false) {
  ...
  if (memoryKey && config.conversationMemory === 1) {
    const history = db.prepare("SELECT role, content FROM conversations WHERE chatId = ? ORDER BY timestamp DESC LIMIT 10").all(memoryKey) as any[];
  ...
  const now = new Date();
  const dateStr = now.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'full', timeStyle: 'short' });
  ...
  // Web Search Trigger
  let searchContext = "";
  const searchKeywords = ["today", "latest", "current", "news", "score", "price", "who is", "what happened", "election", "weather", "match"];
  ...
  const geminiProvider = { name: 'Gemini', key: userGeminiK || systemGeminiK, fn: (p: any, k: any, ctx: any, inst: any) => getGeminiResponse(p, k, config.activeModel, ctx, inst) };
  const groqProvider = { name: 'Groq', key: groqK, fn: (p: any, k: any, ctx: any, inst: any) => getGroqResponse(p, k, config.activeModel, ctx, inst) };
  const orProvider = { name: 'OpenRouter', key: openRouterK, fn: (p: any, k: any, ctx: any, inst: any) => getOpenRouterResponse(p, k, config.activeModel, ctx, inst) };
  const bluesmindsProvider = { name: 'BluesMinds', key: config.bluesmindsApiKey, fn: (p: any, k: any, ctx: any, inst: any) => getBluesMindsResponse(p, k, config.activeModel || 'gemini-1.5-flash', ctx, inst) };

  if (config.aiProvider === 'gemini') {
    providers.push(geminiProvider, bluesmindsProvider, groqProvider, orProvider);
  } else if (config.aiProvider === 'groq') {
    providers.push(groqProvider, bluesmindsProvider, geminiProvider, orProvider);
  } else if (config.aiProvider === 'bluesminds') {
    providers.push(bluesmindsProvider, geminiProvider, groqProvider, orProvider);
  } else {
    providers.push(orProvider, bluesmindsProvider, geminiProvider, groqProvider);
  }

  for (const p of providers) {
    if (p.key && p.key !== 'undefined' && p.key !== 'null' && p.key.length > 5) {
      try {
        const resRaw = await p.fn(prompt, p.key, context, `${timeContext} ${systemPrompt} ${searchContext ? "\n\n" + searchContext : ""}`);
        if (resRaw) {
          const res = cleanAIResponse(resRaw, config);
          
          if (memoryKey && config.conversationMemory === 1) {
            db.prepare("INSERT INTO conversations (chatId, role, content, timestamp) VALUES (?, ?, ?, ?)").run(memoryKey, 'user', prompt, Date.now());
            db.prepare("INSERT INTO conversations (chatId, role, content, timestamp) VALUES (?, ?, ?, ?)").run(memoryKey, 'model', res, Date.now());
          }
          return res;
        }
      } catch (err: any) {
...
async function maybeDeleteCommand(client: TelegramClient | null, message: any, config: any) {
  if (!client || config.autoDeleteCommands !== 1) return;
  
  const chatId = message.chatId?.toString();
  const whitelistString = (config.autoDeleteWhitelist || "").trim();
  const whitelist = whitelistString ? whitelistString.split(',').map((s: string) => s.trim()) : [];
  ...
  const deleteAction = async () => {
    try {
      ...
      console.log(`[BOT] Auto-deleted command message ${message.id} from chat ${chatId}`);
      
      const logId = Math.random().toString(36).substring(2);
      db.prepare("INSERT INTO logs (id, timestamp, message, type) VALUES (?, ?, ?, ?)").run(
        logId, Date.now(), `Auto-deleted command message (${message.id}) from chat ${chatId}`, 'info'
      );
    } catch (e: any) {
      console.error(`[BOT] Failed to auto-delete command message ${message.id}: ${e.message}`);
    }
  };
...
class PermissionManager {
  ...
  static async check(command: string, userId: string, chatId: string, myId: string): Promise<{ allowed: boolean, reason?: string, level: PermissionLevel }> {
    ...
    const blacklisted = (config.blacklistedUsers || "").split(',').map((s: any) => s.trim()).filter(Boolean);
    const whitelisted = (config.whitelistedUsers || "").split(',').map((s: any) => s.trim()).filter(Boolean);

    if (level < PermissionLevel.OWNER && blacklisted.includes(userId)) {
      return { allowed: false, reason: "🚫 You are blacklisted.", level };
    }

    const publicCommands = ['ans', 'music', 'song', 'gif', 'sticker', 'pdf', 'summarize', 'translate', 'ping', 'commands', 'help', 'nsfw', 'confirmage'];
    ...
    if (level === PermissionLevel.SUDO) {
        ...
        if (ownerOnly.includes(cmdName)) return { allowed: false, reason: "👑 Owner only command.", level };
    ...
    if (publicCommands.includes(cmdName)) {
        if (config.publicCommandsEnabled !== 1 && !whitelisted.includes(userId)) {
            return { allowed: false, reason: "🔒 Public commands are globally disabled.", level };
        }

        const groupSettings = db.prepare("SELECT * FROM group_settings WHERE chatId = ?").get(chatId) as any;
        if (groupSettings && groupSettings.publicCommandsEnabled === 0) {
            return { allowed: false, reason: "🔒 Public commands are disabled in this group.", level };
        }
...
class CommandProcessor {
  static async process(client: TelegramClient, message: any, config: any, myId: string, cmdName: string, textRaw: string, handler: (status: SmartStatus) => Promise<void>) {
    const chatId = message.chatId?.toString();
    ...
    if (!check.allowed) {
      if (check.reason && !isMe) {
          await client.sendMessage(message.chatId, { message: check.reason, replyTo: message.id });
      }
      return;
    }
    ...
    // 4. Status Message
    const status = new SmartStatus(client, message.chatId, true, message.replyToMsgId || message.id);
    
    // 5. Execute Command
    try {
      // Log usage
      db.prepare("INSERT INTO command_stats (command, userId, chatId, timestamp) VALUES (?, ?, ?, ?)").run(cmdName, userId, chatId, Date.now());
...
async function handleSummarize(client: TelegramClient, message: any, config: any, status: SmartStatus) {
  ...
  const aiRes = await getAIResponse(`Summarize this text concisely: ${text}`, config, message.chatId.toString(), message.senderId?.toString());
  if (aiRes) {
...
async function handleTranslate(client: TelegramClient, message: any, config: any, status: SmartStatus, args: string) {
  if (!message.replyToMsgId) {
  ...
  const aiRes = await getAIResponse(`Translate this text to ${targetLang}. Only return the translation: ${text}`, config, message.chatId.toString(), message.senderId?.toString());
  if (aiRes) {
...
async function handleGif(client: TelegramClient, message: any, config: any, status: SmartStatus, query: string) {
  ...
  await status.fail("GIF search API not configured. Please use /img for custom generations.");
}
...
class SmartStatus {
  ...
  constructor(client: TelegramClient | null, chatId: any, autoDelete = true, replyTo: number | null = null) {
    this.client = client;
    this.chatId = chatId;
...
async function startServer() {
  ...
  const setIsRunning = (state: boolean) => {
    db.prepare("UPDATE config SET isRunning = ? WHERE id = 1").run(state ? 1 : 0);
    runningLoop = state;
  };

  const addLog = (message: string, type: 'info' | 'success' | 'warn' | 'error' = 'info') => {
    const id = Math.random().toString(36).substring(2);
    db.prepare("INSERT INTO logs (id, timestamp, message, type) VALUES (?, ?, ?, ?)").run(id, Date.now(), message, type);
  };
  ...
  const downloadYoutube = async (url: string, output: string) => {
    ...
    try {
      ...
      addLog(`Primary download failed: ${err.message}. Retrying with alternate settings...`, "warn");
      try {
        const fallbackFlags = { ...commonFlags, format: 'bestaudio/best' };
        await youtubedl(url, fallbackFlags);
      } catch (err2: any) {
        addLog(`Fallback download failed: ${err2.message}. Trying play-dl as last resort...`, "error");
  ...
  async function handleMusicCommand(message: any, text: string, status?: SmartStatus) {
  ...
  async function handleStickerCommand(client: TelegramClient, message: any, status: SmartStatus) {
    if (!message.replyToMsgId) return status.fail('Reply to a message with /stcr');
    await status.update(`⏳ Preparing Quotly sticker...`);

    try {
      const repl = await client.getMessages(message.chatId, { ids: [message.replyToMsgId] });
      if (!repl || repl.length === 0) return status.fail('Could not find reply message.');
      ...
      if (sender) {
        // @ts-ignore
        senderName = sender.firstName ? `${sender.firstName} ${sender.lastName || ''}`.trim() : (sender.username || "User");
      }
      ...
      const cleanText = (target.message || "").replace(/[<>&"']/g, (c: string) => {
        switch (c) {
          case '<': return '&lt;';
          case '>': return '&gt;';
          case '&': return '&amp;';
      ...
      const colors = ['#ff7474', '#74ff74', '#7474ff', '#ffff74', '#ff74ff', '#74ffff'];
      ...
      const wrappedLines = wrapText(target.message || (target.media ? "[Media]" : ""), wrapLimit).slice(0, 15);
  ...
  async function handlePdfCommand(client: TelegramClient, message: any, status: SmartStatus) {
    if (!message.replyToMsgId) return status.fail('Reply to a message with /pdf');
...
async function maybeHandleAutoReply(client: TelegramClient, message: any, config: any, myId: string, myUsername: string) {
    ...
    if (config.autoReplyMention === 1) {
      ...
      if (replyMsgId) {
        try {
          ...
          const repliedMsg = await client.getMessages(target, { ids: [replyMsgId] });
          if (repliedMsg && repliedMsg.length > 0 && (repliedMsg[0].out || repliedMsg[0].senderId?.toString() === myId)) {
            isReplyToMe = true;
          }
        } catch (e) {
          console.error(`[AI-Auto] Error fetching replied message for chat ${chatIdStr}:`, e);
        }
      }
      
      if (isMentioned || isReplyToMe) {
        console.log(`[AI-Auto] Triggered in group! Mentioned: ${isMentioned}, ReplyToMe: ${isReplyToMe}`);
        shouldReply = true;
      } else {
  ...
  // Blacklist check
  const blacklist = (config.autoReplyBlacklist || "").split(',').map((s: string) => s.trim()).filter(Boolean);
  if (blacklist.includes(senderId) || blacklist.includes(chatIdStr)) {
    console.log(`[AI-Auto] Blocked by blacklist: ${senderId} or ${chatIdStr}`);
    return;
  }

  // Whitelist check (if not empty)
  const whitelist = (config.autoReplyWhitelist || "").split(',').map((s: string) => s.trim()).filter(Boolean);
  ...
  if (isPrivate && config.nsfwEnabled === 1) {
    const userPref = db.prepare("SELECT nsfwEnabled, ageConfirmed FROM user_nsfw_prefs WHERE userId = ?").get(senderId) as any;
  ...
  if (!modResult.safe) {
    console.log(`[AI-Auto] NSFW Content Violation by ${senderId}: ${modResult.reason}`);
    const nsfwLogId = Math.random().toString(36).substring(2);
    db.prepare("INSERT INTO nsfw_logs (id, timestamp, userId, chatId, message, violation) VALUES (?, ?, ?, ?, ?, ?)")
      .run(nsfwLogId, Date.now(), senderId, chatIdStr, text, modResult.reason);
    
    if (isPrivate) {
      await client.sendMessage(message.chatId, { message: "⚠️ **Moderation:** Your message violates our safety guidelines. NSFW mode has been temporarily restricted for this conversation.", replyTo: message.id });
    }
    return;
  }

  console.log(`[AI-Auto] Processing reply for ${chatIdStr} (NSFW: ${isNSFWActive})...`);
  addLog(`Processing auto-reply for ${chatIdStr} (NSFW: ${isNSFWActive})`, "info");
  ...
  console.log(`[AI-Auto] AI reply triggered for ${chatIdStr}. Delay: ${actualDelay/1000}s`);
  ...
  setTimeout(async () => {
    try {
      ...
      const aiRes = await getAIResponse(text, config, chatIdStr, senderId, isNSFWActive);
      
      if (aiRes && client) {
        // Post-generation moderation
        const responseMod = await moderateContent(aiRes);
        const formatted = formatAiMessage(responseMod.safe ? aiRes : "I cannot fulfill that request due to safety restrictions.");
        ...
        await status.finish("❌ **AI Error:** Could not generate a response. Please check your keys or try again later.");
      }
    } catch (e: any) {
  ...
  const loadTelethon = async () => {
    ...
    if (!apiId || !apiHash || !stringSessionStr) {
      addLog("Telegram credentials missing. Please set API ID, Hash, and Session in Settings.", "warn");
      console.warn("[BOT] Telegram credentials missing. apiId:", !!apiId, "apiHash:", !!apiHash, "session:", !!stringSessionStr);
      return false;
    }
    ...
    try {
      ...
      client = new TelegramClient(stringSession, parseInt(apiId.toString()), apiHash, {
        connectionRetries: 5,
        useWSS: false
      });
        ...
        if (config.nsfwEnabled === 1 && message.isPrivate && !isMe) {
          if (text === '/nsfw on' || text === '.nsfw on') {
            ...
            if (userPref?.ageConfirmed === 1) {
              db.prepare("INSERT OR REPLACE INTO user_nsfw_prefs (userId, nsfwEnabled, ageConfirmed, updatedAt) VALUES (?, 1, 1, ?)").run(senderId, Date.now());
              await client.sendMessage(message.chatId, { message: "🔞 **Mature Mode Activated.** Your AI friend is now in mature chat mode. Rest assured, this is a private conversation between consenting adults only.", replyTo: message.id });
            } else {
              await client.sendMessage(message.chatId, { 
          ...
          if (text === '/nsfw off' || text === '.nsfw off') {
            db.prepare("INSERT OR REPLACE INTO user_nsfw_prefs (userId, nsfwEnabled, ageConfirmed, updatedAt) VALUES (?, 0, 1, ?)").run(senderId, Date.now());
            await client.sendMessage(message.chatId, { message: "✅ **Mature Mode Deactivated.** Returning to standard conversational mode.", replyTo: message.id });
            return;
          }
          if (text === '/nsfw status' || text === '.nsfw status') {
            const userPref = db.prepare("SELECT nsfwEnabled FROM user_nsfw_prefs WHERE userId = ?").get(senderId) as any;
            const status = userPref?.nsfwEnabled === 1 ? "Active 🔞" : "Inactive 👤";
            await client.sendMessage(message.chatId, { message: `🔞 **Mature Mode Status:** ${status}`, replyTo: message.id });
            return;
          }
          if (text === '/confirmage' || text === '.confirmage') {
            db.prepare("INSERT OR REPLACE INTO user_nsfw_prefs (userId, nsfwEnabled, ageConfirmed, updatedAt) VALUES (?, 1, 1, ?)").run(senderId, Date.now());
            await client.sendMessage(message.chatId, { message: "✅ **Age Confirmed.** Mature Mode has been enabled for you. Type `/nsfw off` anytime to disable it.", replyTo: message.id });
            return;
          }
        }
...
_Visit the dashboard for advanced configuration._`;
  ...
  setInterval(async () => {
    try {
      ...
      addLog(`Failed to connect to Telegram: ${e?.message || 'Unknown error'}`, "error");
      client = null;
      return false;
    }
  };
      ...
      while (getIsRunning()) {
        ...
        if (client) {
          try {
            ...
            addLog(`Sent to ${target.name}: "${msg.text.substring(0, 20)}..."`, "success");
          } catch (e: any) {
             addLog(`Failed sending to ${target.name}: ${e?.message}`, "error");
          }
        } else {
             // Mock
             addLog(`[MOCK] Sent to ${target.name}: "${msg.text.substring(0, 20)}..."`, "success");
        }

        const delaySec = Math.floor(Math.random() * (config.maxDelaySeconds - config.minDelaySeconds + 1) + config.minDelaySeconds);
